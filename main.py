    ...
    if opt.test:
        ...
        for action in actions:
            ...
            test_loader = DataLoader(
                dataset=Human36M(
                    actions=action, data_path=opt.data_dir, use_hg=opt.use_hg, is_train=False),
                batch_size=opt.test_batch,
                shuffle=False,
                num_workers=opt.job,
                pin_memory=True)
            _, err_test = test(test_loader, model, criterion,
                               stat_3d, procrustes=opt.procrustes)
...
def train(train_loader, model, criterion, optimizer,
    ...
    for i, (inps, tars) in enumerate(train_loader):
        ...
        if glob_step % lr_decay == 0 or glob_step == 1:
            lr_now = utils.lr_decay(
                optimizer, glob_step, lr_init, lr_decay, gamma)
...
def test(test_loader, model, criterion, stat_3d, procrustes=False):
    ...
    for i, (inps, tars) in enumerate(test_loader):
        ...
        targets_unnorm = data_process.unNormalizeData(
            tars.data.cpu().numpy(), stat_3d['mean'], stat_3d['std'], stat_3d['dim_use'])
        outputs_unnorm = data_process.unNormalizeData(outputs.data.cpu(
        ).numpy(), stat_3d['mean'], stat_3d['std'], stat_3d['dim_use'])
